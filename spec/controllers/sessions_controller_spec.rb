...
describe CASino::SessionsController do
  ...
  describe 'POST "create"' do
    ...
    context 'with a valid login ticket' do
      ...
      let!(:user) { FactoryBot.create :user, authenticator: 'static', username: username, locked_until: locked_until }
    ...
    context 'with an existing ticket-granting ticket' do
      ...
      context 'with an active authenticator' do
        ...
        context 'with a valid OTP' do
          before(:each) do
            ROTP::TOTP.any_instance.should_receive(:verify).with(otp, drift_behind: 30).and_return(true)
          end
        ...
        context 'with an invalid OTP' do
          before(:each) do
            ROTP::TOTP.any_instance.should_receive(:verify).with(otp, drift_behind: 30).and_return(false)
          end
